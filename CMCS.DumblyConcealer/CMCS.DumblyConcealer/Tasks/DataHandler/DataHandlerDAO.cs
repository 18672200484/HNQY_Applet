...
namespace CMCS.DumblyConcealer.Tasks.CarSynchronous
{
	...
	public class DataHandlerDAO
	{
		...
		public void SyncStorageTotal(Action<string, eOutputType> output)
		{
			...
			#region 出厂车数
			CommonDAO.GetInstance().SetSignalDataValue(GlobalVars.MachineCode_HomePage_1, "总体信息_汽车出厂车数", CarCount_GQCH.ToString());
			#endregion
			...
			#region 翻车车数
			sql = string.Format("select count(id) from fultbtrainweighter where trunc(GrossDate)=trunc(sysdate)");
			if (dt.Rows.Count > 0)
			{
				HCRCCS = int.Parse(dt.Rows[0][0].ToString() ?? "0");
			}
			CommonDAO.GetInstance().SetSignalDataValue(GlobalVars.MachineCode_HomePage_1, "火车_入厂车数", HCRCCS.ToString());

			sql = "select count(id) from  cmcstbtrainrecognition t where trunc(crosstime)=trunc(sysdate) and machinecode like '%1%' and t.direction ='进'";
			dt = Dbers.GetInstance().SelfDber.ExecuteDataTable(sql);
			if (dt.Rows.Count > 0)
			{
				YFC1 = int.Parse(dt.Rows[0][0].ToString() ?? "0");
			}
			CommonDAO.GetInstance().SetSignalDataValue(GlobalVars.MachineCode_HomePage_1, "火车_#1已翻车数", YFC1.ToString());

			sql = "select count(id) from  cmcstbtrainrecognition t where trunc(crosstime)=trunc(sysdate) and machinecode like '%2%' and t.direction ='进'";
			dt = Dbers.GetInstance().SelfDber.ExecuteDataTable(sql);
			if (dt.Rows.Count > 0)
			{
				YFC2 = int.Parse(dt.Rows[0][0].ToString() ?? "0");
			}
			CommonDAO.GetInstance().SetSignalDataValue(GlobalVars.MachineCode_HomePage_1, "火车_#2已翻车数", YFC2.ToString());
			CommonDAO.GetInstance().SetSignalDataValue(GlobalVars.MachineCode_HomePage_1, "火车_已翻车数", (YFC1 + YFC2).ToString());
			#endregion
