...
namespace CMCS.CarTransport.BeltSampler.Frms
{
	public partial class FrmBeltSampler : DevComponents.DotNetBar.Metro.MetroForm
	{
		...
		{
			List<View_RCSampling> list = beltSamplerDAO.GetViewRCSampling("where BatchType='火车' and SamplingDate >= '" + DateTime.Now.AddDays(-3).ToString("yyyy-MM-dd") + "' order by SamplingDate desc");
			superGridControl.PrimaryGrid.DataSource = list;
		}
	}
}
