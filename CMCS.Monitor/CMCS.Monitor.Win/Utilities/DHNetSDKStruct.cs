...
namespace DHNetSDK
{
    ...
    #region <<目标检测模块配置 - 相关结构 Object Detection Module Configuration--Related Structure>>
    ...
    #region <<全局配置视频分析 - 相关结构 Global Configuration Video Analysis--Related structure>>
    ...
    #region <<在智能交通三个配置：场景，模块，交通规则配置 Three configuration in intelligent transportation：Scene、Module、traffic rules Configuration>>
    ...
    #region <<事件类型事件IVS TRAFFICGATE（交通监视事件）相应的数据块说明信息 Event Type EVENT_IVS_TRAFFICGATE(Traffic Monitoring Event)The Corresponding Data Block Description Information>>
    ...
    #region 结构化 - 情报分析事件类型 structures--Intelligent Analysis Event Type
    ...
    #region <<结构 - 功能中设置的指令--- CLIENT QueryNew的SystemInfo structure--instructions in function set --- CLIENT_QueryNewSystemInfo>>
    ...
    #region <<结构 - 设置指令CLIENT_GetNewDevConfig＆CLIENT_SetNewDevConfig接口 structure--setting instructions CLIENT_GetNewDevConfig & CLIENT_SetNewDevConfig interface>>
    /// <summary>
    /// video format
    /// </summary>
    ...
    #region <<交通检查点的事件 - 相应的规则配置 traffic inspection spot event – corresponding rule configuration >>
    ...
    #region <<报警运输 - 客户端开始听 Alarm transportation - CLIENT_StartListen>>
    ...
    #region <<智能交通 - 下载图像相关的结构 Intelligent transportation - download images related structure
