...
namespace CMCS.Monitor.Win.Frms
{
    public partial class FrmEquInfHitch : MetroForm
    {
        ...
        private void InitForm()
        {
            ...
            cmbEquipment.DataSource = Dbers.GetInstance().SelfDber.Entities<CmcsCMEquipment>("where parentid in (select a.id from cmcstbcmequipment a where a.equipmentname in ('全自动制样机','智能存样柜','气动传输')) order by nodecode");
            //cmbEquipment.SelectedIndex = 0;
            cmbEquipment.SelectedValue = string.IsNullOrEmpty(this.MachineCode) ? "气动传输" : this.MachineCode;
