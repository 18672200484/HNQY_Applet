...
namespace CMCS.Monitor.Win.Frms.Sys
{
	public partial class FrmMainFrame : MetroForm
	{
		...
		public static string CarSampleNum = "1";
		...
		/// 打开汽车机械采样监控
		/// </summary>
		public void OpenCarSampler(string equNum)
		{
			FrmMainFrame.CarSampleNum = equNum;
		...
		public void OpenEquInfHitch(string Param)
		{
			string uniqueKey = FrmEquInfHitch.UniqueKey;
			this.BeginInvoke((Action)(() =>
			{
				...
				{
					FrmEquInfHitch frm = new FrmEquInfHitch(Param);
					frm.ShowDialog();
					//FrmMainFrame.superTabControlManager.CreateTab(frm.Text, uniqueKey, frm, false);
				}
				else
		...
		private void btnOpenCarSampler_Click(object sender, EventArgs e)
		{
			...
			OpenCarSampler((sender as ButtonX).CommandParameter.ToString());
		}
		...
		/// 打开存样传输记录
		/// </summary>
		/// <param name="enttiy"></param>
		public void OpenCygControlCmd()
		{
			this.BeginInvoke((Action)(() =>
			{
				FrmAutoCupboardRecord frm = new FrmAutoCupboardRecord();
				frm.ShowDialog();
			}));
		}
		#endregion
		...
		private void InitEquipmentStatus()
		{
			...
			List<CmcsCMEquipment> list = commonDAO.GetChildrenMachinesByCode("汽车机械采样机");
		...
		public void ShowEquInfHitch()
		{
			...
			if (listResult.Count > 0)
			{
				foreach (InfEquInfHitch item in listResult)
				{
					...
					sbHitchDescribe.Append(item.HitchTime.ToString("HH:mm") + "   " + item.MachineCode + "- " + item.HitchDescribe + "<br>");
		...
		private void ShiPin_Click(object sender, EventArgs e)
		{
			OpenShiPing();
		}
		...
		public void OpenShiPing()
		{
			this.InvokeEx(() =>
			{
				string uniqueKey = FrmVidioPreview.UniqueKey;

				if (FrmMainFrame.superTabControlManager.GetTab(uniqueKey) == null)
				{
					FrmVidioPreview frm = new FrmVidioPreview();
					FrmMainFrame.superTabControlManager.CreateTab(frm.Text, uniqueKey, frm, false);
				}
				else
					FrmMainFrame.superTabControlManager.ChangeToTab(uniqueKey);
		...
		/// 打开翻车记录
		/// </summary>
		public void OpenTrainRecognition()
		{
			this.InvokeEx(() =>
			{
				FrmTrainRecognition frm = new FrmTrainRecognition();
				frm.ShowDialog();
			});
		}
