...
{
	public partial class FrmTrainRecognition : MetroForm
	{
		/// <summary>
		/// 窗体唯一标识符
		/// </summary>
		public static string UniqueKey = "FrmTrainRecognition";
		string MachineCode = string.Empty;
		public FrmTrainRecognition(string machineCode = "")
		{
			InitializeComponent();
		...
		public void BindData()
		{
			...
			List<CmcsTrainRecognition> list = Dbers.GetInstance().SelfDber.ExecutePager<CmcsTrainRecognition>(PageSize, CurrentIndex, tempSqlWhere + " order by CreateDate desc");
		...
		private void btnSearch_Click(object sender, EventArgs e)
		{
			...
			{
				SqlWhere += " and MachineCode like '%" + cmbEquipment.Text + "%'";
			}
			if (!String.IsNullOrEmpty((String)dateTimeInput1.Text))
			{
				SqlWhere += " and CrossTime>= to_date('" + dateTimeInput1.Value.ToString("yyyy-MM-dd HH:mm:ss") + "','yyyy-mm-dd hh24:mi:ss') ";
			}
			if (!String.IsNullOrEmpty((String)dateTimeInput2.Text))
			{
				SqlWhere += " and CrossTime< to_date('" + dateTimeInput2.Value.ToString("yyyy-MM-dd HH:mm:ss") + "','yyyy-mm-dd hh24:mi:ss') ";
			}
