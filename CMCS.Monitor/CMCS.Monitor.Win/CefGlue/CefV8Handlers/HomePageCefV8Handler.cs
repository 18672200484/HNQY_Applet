		...
		protected override bool Execute(string name, CefV8Value obj, CefV8Value[] arguments, out CefV8Value returnValue, out string exception)
		{
			...
			switch (name)
			{
				...
				case "OpenTrainMachinerySampler":
					...
					if (paramSampler == "1")
					{
						CefV8Context.GetCurrentContext().GetBrowser().SendProcessMessage(CefProcessId.Browser, CefProcessMessage.Create("OpenCarSampler1"));
					}
					else
					{

						CefV8Context.GetCurrentContext().GetBrowser().SendProcessMessage(CefProcessId.Browser, CefProcessMessage.Create("OpenCarSampler2"));
					}
					break;
				// 打开全自动制样机监控
				case "OpenAutoMaker":
				...
				case "OpenLaboratory":
					...
					break;
				//  打开翻车记录
				case "OpenTrainRecognition":
					CefV8Context.GetCurrentContext().GetBrowser().SendProcessMessage(CefProcessId.Browser, CefProcessMessage.Create("OpenTrainRecognition"));
					break;
				default:
					returnValue = null;
					break;
