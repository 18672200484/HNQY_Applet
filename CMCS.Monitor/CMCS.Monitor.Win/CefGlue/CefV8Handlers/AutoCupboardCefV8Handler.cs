        ...
        protected override bool Execute(string name, CefV8Value obj, CefV8Value[] arguments, out CefV8Value returnValue, out string exception)
        {
            ...
            switch (name)
            {
                ...
                case "GetHitchs":
                    ...
                    cefMsg = CefProcessMessage.Create("OpenEquInfHitch");
                    cefMsg.Arguments.SetSize(1);
                    cefMsg.Arguments.SetString(0, machineCode);
                    CefV8Context.GetCurrentContext().GetBrowser().SendProcessMessage(CefProcessId.Browser, cefMsg);
                    break;
                case "ShowCygControlCmd":
                    cefMsg = CefProcessMessage.Create("OpenControlCmd");
                    cefMsg.Arguments.SetSize(1);
                    CefV8Context.GetCurrentContext().GetBrowser().SendProcessMessage(CefProcessId.Browser, cefMsg);
                    break;
                default:
